...
func seedAttributedHistory(t *testing.T, roles *sql.DB, session domain.Session) (string, string) {
	t.Helper()
	const rootID, answerID = "history-user", "history-role-answer"
	_, err := roles.Exec(`INSERT INTO messages
	...
	_, err = roles.Exec(`INSERT INTO message_parts(id,message_id,ordinal,block_kind,payload_json) VALUES
		('history-part-1',?,0,'text','{"text":"Initial question"}'),
		('history-part-2',?,0,'text','{"text":"Prior finding"}')`, rootID, answerID)
	require.NoError(t, err)
	_, err = roles.Exec(`UPDATE sessions SET active_leaf_message_id=? WHERE id=?`, answerID, session.ID)
	require.NoError(t, err)
	_, err = roles.Exec(`UPDATE session_branches SET leaf_message_id=? WHERE id=?`, answerID, *session.ActiveBranchID)
	require.NoError(t, err)
...
func TestContextProjectorRoomPreservesSpeakerAndExcludesControlFacts(t *testing.T) {
	...
	require.NoError(t, err)
	projected, err := (&store.ContextProjector{DB: roles}).ProjectAndFreeze(ctx, submission.Run)
	require.NoError(t, err)
	...
	require.NoError(t, roles.QueryRow(`SELECT context_snapshot_digest FROM agent_runs WHERE id=?`, submission.Run.ID).Scan(&frozenDigest))
	assert.Equal(t, projected.Digest, frozenDigest)
	projectedAgain, err := (&store.ContextProjector{DB: roles}).ProjectAndFreeze(ctx, submission.Run)
	require.NoError(t, err)
...
func TestContextProjectorQuotesParticipantPromptInjectionAsBoundedData(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = roles.Exec(`UPDATE message_parts SET payload_json=? WHERE message_id=?`, string(payload), answerID)
	require.NoError(t, err)
...
func TestContextProjectorRejectsMissingParticipantInviteFact(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = roles.Exec(`DELETE FROM message_parts WHERE block_kind='room_control' AND message_id IN
		(SELECT id FROM messages WHERE session_id=?)`, session.ID)
	require.NoError(t, err)
	_, err = (&store.ContextProjector{DB: roles}).ProjectAndFreeze(ctx, submission.Run)
	require.Error(t, err)
