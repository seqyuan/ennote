...
func (t *PublishArtifactTool) Execute(ctx context.Context, call domain.ToolCall) domain.ToolResult {
	var args struct {
		Path string `json:"path"`
		Name string `json:"name"`
	}
	if err := json.Unmarshal(call.Arguments, &args); err != nil {
		return errorResult(call, fmt.Errorf("invalid publish_artifact arguments: %w", err))
	}
	if strings.TrimSpace(args.Path) == "" {
		return errorResult(call, fmt.Errorf("path must not be empty"))
	}
	...
	if !info.Mode().IsRegular() {
		return errorResult(call, fmt.Errorf("artifact source must be a regular file"))
	}
	...
	return domain.ToolResult{ToolCallID: call.ID, ToolName: call.Name,
		...
		Artifacts: []domain.ArtifactReference{reference}}
}
