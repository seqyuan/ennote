...
func (t *EditTool) Execute(ctx context.Context, call domain.ToolCall) domain.ToolResult {
	var args struct {
	...
	if err := json.Unmarshal(call.Arguments, &args); err != nil {
		return errorResult(call, fmt.Errorf("invalid edit arguments: %w", err))
	}
	if args.OldText == "" {
		return errorResult(call, fmt.Errorf("oldText must not be empty"))
	}
	...
	if info.Size() > maxBytes {
		return errorResult(call, fmt.Errorf("file exceeds edit limit of %d bytes", maxBytes))
	}
	...
	if count != 1 {
		return errorResult(call, fmt.Errorf("oldText must match exactly once; matched %d times", count))
	}
	...
	return domain.ToolResult{ToolCallID: call.ID, ToolName: call.Name, Content: "edited " + display}
}
